<header class="header">
    ...
    <nav class="navbar">
        ...
        <a class="<?php if (!empty($user)) {
            if ($user->id == $_SESSION['USER']->id) {
                active('profile');
            }
        } ?>"
            href="<?= ROOT ?>/profile/index/<?= $_SESSION['USER']->id ?>">Profile</a>
        <a class="<?php active('queries'); ?>" href="<?= ROOT ?>/queries/index">Queries</a>
        <a class="<?php active('answers'); ?>" href="<?= ROOT ?>/answers/all/<?= $_SESSION['USER']->id ?>">Answers</a>
